...
- (void) startDownloadImage
{
    ...
    [[DBImageViewCache cache] imageForURL:_remoteImage.imageURL found:^(UIImage *image) {
        ...
        self->_currentRequest = self->_remoteImage.imageRequest;
        
        [self->_currentRequest downloadImageWithSuccess:^(UIImage *image, NSHTTPURLResponse *response) {
            [self stopSpinner];
            
            self->_imageView.image = image;
            self->_currentRequest = nil;
            self->_imageWithPath = nil;
        } error:^(NSError *error) {
            [self stopSpinner];
            self->_currentRequest = nil;
            self->_imageWithPath = nil;
