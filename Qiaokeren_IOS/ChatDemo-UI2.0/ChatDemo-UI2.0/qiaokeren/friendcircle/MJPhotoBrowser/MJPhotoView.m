...
//	}
    ...
    if (_photo.firstShow) { // 第一次显示的图片
        ...
        [UIView animateWithDuration:  0.3  animations:^{
            ...
            // 设置底部的小图片
            self->_photo.srcImageView.image = self->_photo.placeholder;
            [self photoStartLoad];
        }];
    } else {
...
- (void)hide
{
    ...
    [UIView animateWithDuration:duration + 0.1 animations:^{
        self->_imageView.frame = [self->_photo.srcImageView convertRect:self->_photo.srcImageView.bounds toView:nil];
