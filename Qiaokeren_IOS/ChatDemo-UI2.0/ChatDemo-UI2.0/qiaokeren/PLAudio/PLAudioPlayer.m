...
//            [VoiceConverter ConvertAmrToWav:fileName wavSavePath:wavSavePath];
    ...
    if (error) {
        ...
        if (flag) {
            ...
            dispatch_source_set_event_handler(self.playTimer, ^{
                [self->_player updateMeters];
                self.playMeters([self->_player averagePowerForChannel:0]);
            });
