...
@interface JZContributionVC ()<JZDataHandleDelegate>
...
@implementation JZContributionVC
...
-(void)sendQst{
    ...
    JZgetBillInfo_c * getMsg = [JZgetBillInfo_c Jz_initialize];
    
    //[UtilsAll getFormatTime]
    //[QiaokerenApplication getAccountNumber]
    [getMsg initWithInfo:@"请求订货账单"
                  UserID:[QiaokerenApplication getAccountNumber]
              UploadTime:[UtilsAll getFormatTime]
                  BillID:@"0"
            SearchUserID:[QiaokerenApplication getAccountNumber]
          SearchUpUserID:@"0"
                PageSize:@"100"
               PageIndex:@"0"];
  
    [dh sendObject:[getMsg toDictionary] time:@"0" protocol:1 label:@"JZgetBillInfo_c"];
}
...
//                NSLog(@"------------------%@",json);
        ...
        if ([[dictionary objectForKey:@"Mark"]isEqualToString:@"1"]) {
            ...
            for (int i=0; i<[array count]; i++) {
              ...
              BillSelectData * billdata = [BillSelectData Jz_initialize];
                [billdata initWithInfo:@"订货账单信息"
...
#pragma mark-  basicView basicView basicView
#pragma mark-  basicView basicView basicView
#pragma mark-  basicView basicView basicView
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    for (int i=0;i<_goodsdata.count;i++){
        ...
        NSLog(@"++++++%@",[[_data objectAtIndex:indexPath.row] goodsid]);
        NSLog(@"------%@",[goodinfodata goodsid]);
        
        if ([[[_data objectAtIndex:indexPath.row] goodsid] isEqualToString:[goodinfodata goodsid]]){
            goodname = [goodinfodata goodsname];
            tolcredit =[NSString stringWithFormat:@"%d",[[goodinfodata QCredit] intValue]*[[[_data objectAtIndex:indexPath.row] goodsnumber]intValue]];
