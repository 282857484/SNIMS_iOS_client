...
- (id)initPageViewWithFrame:(CGRect)frame views:(NSArray *)views
{
    self = [super initWithFrame:frame];
    if (self) {
        selectionType = JZScrollViewSelecttionTypeTap;
        sourceArr = views;
...
//    timer = [NSTimer scheduledTimerWithTimeInterval:6.0 target:self selector:@selector(nextPage:) userInfo:nil repeats:YES];
...
{
    if (selectionType != JZScrollViewSelecttionTypeTap) {
        return;
    }
