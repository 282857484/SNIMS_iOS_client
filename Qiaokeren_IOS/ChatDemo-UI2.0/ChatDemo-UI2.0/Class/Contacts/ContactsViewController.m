...
#import "QiaokerenApplication.h"
@interface ContactsViewController ()<UITableViewDataSource, UITableViewDelegate, UISearchBarDelegate, UISearchDisplayDelegate, UIActionSheetDelegate, BaseTableCellDelegate, SRRefreshDelegate, IChatManagerDelegate,JZDataHandleDelegate>
{
...
+(void)setDataArray:(NSMutableArray *)data2{
    if (!(dataArray == data2)) {
        dataArray = data2;
    }
}
...
+(NSString*)getNickNameByUserId:(NSString*)userid
{
    ...
    for (int i =0;i<dataArray.count;i++) {
        JZgetNickName_S* Na = [dataArray objectAtIndex:i];
        if([userid isEqualToString:Na.UserID]){
            str = [NSString stringWithFormat:@"%@",Na.NickNumber];
            break;
        }
    }
    return str;
...
- (void)dealLabel:(NetUnit*)nUnit{
    ...
    if ([nUnit->cLabel isEqualToString:@"ContactsViewController"]) {
        ...
        if ([[dictionary objectForKey:@"Mark"]isEqualToString:@"1"]) {
            NSString *str = [dictionary objectForKey:@"NickNumber"];
            if(str.length < 20){
                return;
            }
            //            NSLog(@"NICkNUMBER:%@",str);
            NSDictionary*dic;
            NSArray* array = [NSJSONSerialization JSONObjectWithData:[[dictionary objectForKey:@"NickNumber"] dataUsingEncoding:NSUTF8StringEncoding] options:NSJSONReadingMutableContainers error:nil] ;
            for (int i=0; i<[array count]; i++) {
                dic = [array  objectAtIndex:i];
                JZgetNickName_S *Nicks = [JZgetNickName_S Jz_initialize];
                [Nicks initWithInfo:@"用户"
                             UserID:[[NSString alloc] initWithFormat:@"%@",[dic objectForKey:@"id"]]
                         UploadTime:@"0"
                               Mark:@"0"
                            Content:@"0"
                         NickNumber:[dic objectForKey:@"nick"]
                         ANPairList:@"0"];
                [data addObject:Nicks];
            }
            
            [ContactsViewController setDataArray:data];
...
- (EMSearchDisplayController *)searchController
{
    if (_searchController == nil) {
        ...
        [_searchController setCellForRowAtIndexPathCompletion:^UITableViewCell *(UITableView *tableView, NSIndexPath *indexPath) {
            ...
            NSString *url = [QiaokerenApplication getTouXiangUrlUserID:buddy.username];
            [cell.imageView setImageFromUrl:url
                                 completion:^(void) {}];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0 && indexPath.row == 0) {
        ...
        if (indexPath.section == 0 && indexPath.row == 1) {
            ...
            NSString* Name = [ContactsViewController getNickNameByUserId:buddy.username];
...
- (void)reloadDataSource
{
    ...
    for(EMBuddy *buddy in self.contactsSource){
        if(![buddy.username isEqual:@""]){
            str = [NSString stringWithFormat:@"%@#%@",buddy.username,str];
        }
    }
    JZgetNickName_C *getNickName= [JZgetNickName_C Jz_initialize];
    [getNickName initWithInfo:@"请求获取好友昵称"
    ...
    [jzDataHandle sendObject:[getNickName toDictionary] time:[UtilsAll getFormatTime] protocol:1 label:@"ContactsViewController"];
