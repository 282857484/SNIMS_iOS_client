...
- (void)setMessageModel:(MessageModel *)model
{
    ...
    if (model.messageType != eMessageTypeChat) {
        ...
        NSString *url = [QiaokerenApplication getTouXiangUrlUserID:model.username];
        [self.headImageView setImageFromUrl:url
                                 completion:^(void) {}];
    }else{//显示非群聊的头像
        if (self.messageModel.isSender) {
            NSString *url = [QiaokerenApplication getTouXiangUrlUserID:[QiaokerenApplication getAccountNumber]];
            [self.headImageView setImageFromUrl:url
                                     completion:^(void) {}];
