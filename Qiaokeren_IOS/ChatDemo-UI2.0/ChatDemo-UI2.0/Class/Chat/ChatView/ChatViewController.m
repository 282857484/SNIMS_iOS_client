...
@interface ChatViewController ()<UITableViewDataSource, UITableViewDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate, SRRefreshDelegate, IChatManagerDelegate, DXChatBarMoreViewDelegate, DXMessageToolBarDelegate, LocationViewDelegate, EMCDDeviceManagerDelegate,EMCallManagerDelegate,  iOSUploadFileCallBack, JZDataHandleDelegate>
{
    ...
    // 是否处于讲课录音模式
    BOOL _isInRecordModel;
    UIButton *_recordButton;
    NSString *_thisClassId;
}
...
@property (nonatomic) BOOL isInRecordModel;
@property (strong, nonatomic) UIButton *recordButton;
@property (strong, nonatomic) NSString *thisClassId;
@end
...
#warning 以下三行代码必须写，注册为SDK的ChatManager的delegate
    ...
    // 是否是level大于3级，是否是群聊，是否是群主
    if ([[[QiaokerenApplication getUserSelectData] qlevel] integerValue] > 3) {
        if (_isChatGroup) {
            EMGroup * chatGroup = [EMGroup groupWithId:_chatter];
            NSDictionary *loginInfo = [[[EaseMob sharedInstance] chatManager] loginInfo];
            NSString *loginUsername = [loginInfo objectForKey:kSDKUsername];
            if ([chatGroup.owner isEqualToString:loginUsername]) {
                [self setupBarRecordVoiceModel];
            }
        } else {
            [self setupBarRecordVoiceModel];
        }
...
// 添加的录音键
- (void)setupBarRecordVoiceModel{
    CGRect r = [ UIScreen mainScreen ].applicationFrame;
    _recordButton = [[UIButton alloc]initWithFrame:CGRectMake(r.size.width - 54, 80, 44, 44)];
    [_recordButton setImage:[UIImage imageNamed:@"back.png"] forState:UIControlStateNormal];
    [_recordButton addTarget:self action:@selector(recordBack) forControlEvents:UIControlEventTouchUpInside];
    //    recordButton.contentEdgeInsets = UIEdgeInsetsMake(0,10, 0, 0);
    [_recordButton setContentEdgeInsets:UIEdgeInsetsMake(0,10, 0, 0)];
...
//-(void) recordBack{
//    
//    if (_isInRecordModel == NO) {
//        _isInRecordModel = YES;
//        
//        [self showHint:@"您已进入讲课录制模式！"];
//        [_recordButton setImage:[UIImage imageNamed:@"delete"] forState:(UIControlStateNormal)];
//        [self sendAddLesson_C];
//    } else {
//        _isInRecordModel = NO;
//        [self showHint:@"您已进入普通聊天模式！"];
//        [_recordButton setImage:[UIImage imageNamed:@"back.png"] forState:(UIControlStateNormal)];
//    }
//}
//-(void) sendAddLesson_C{
//    addLesson_C *addLesson = [addLesson_C instance2];
//    NSString * time = [UtilsAll getFormatTime];
//    //    NSString * introduce = [NSString stringWithFormat:@"由[%@]讲解的课程",[[QiaokerenApplication getUserSelectData]qusername]]；
//    [addLesson initWithInfo:@"addLesson_C"
//                          p:[addLesson p]
...
//                     userid:[[QiaokerenApplication getUserSelectData]quserid]
//                  timestamp:time
//                   username:[[QiaokerenApplication getUserSelectData]qusername]
//                  userlevel:[[QiaokerenApplication getUserSelectData]qlevel]
//                 classlevel:[[QiaokerenApplication getUserSelectData]qlevel]
//                      place:[UtilsAll getLocation]
//                  introduce:[NSString stringWithFormat:@"由[%@]讲解的课程",[[QiaokerenApplication getUserSelectData]qusername]]
//                      topic:[[QiaokerenApplication getUserSelectData]qusername] // 群名称作为主题
//                       type:@"0" //
//                       suit:@"所有人"];
//    NSDictionary * dictionary = [addLesson toDictionary];
//    JZDataHandle * jzDataHandle = [JZDataHandle initdatahandle];
//    jzDataHandle.JZDatadelegate = self;
//    [jzDataHandle sendObject:dictionary time:time protocol:1 label:@"addLesson"];
//}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row < [self.dataSource count]) {
        ...
        if ([obj isKindOfClass:[NSString class]]) {
            ...
            //点击事件
            cell.headImageView.restorationIdentifier = model.nickName;
            ...
            [cell.headImageView addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(showOtherPersonInfoInChatView:)] ];
...
-(void)showOtherPersonInfoInChatView:(UITapGestureRecognizer *)imageTap{
    PersonInfoViewController *view = [[PersonInfoViewController alloc] init];
    [view setPersonUserId:imageTap.view.restorationIdentifier];
...
-(void)sendAudioMessage:(EMChatVoice *)voice
{
    ...
    if([[[QiaokerenApplication getUserSelectData] qlevel] integerValue] > 3){
        if(_isInRecordModel){
            NSString * theTime = [UtilsAll getFormatTime];
            // 将音频抓取
            [GetMessageProtocol getVoiceMessage:voice isGroup:_isChatGroup isError:NO isMaster:YES Model:YES infoDelegate:self BuildTime:theTime];
            [self sendAddLessonContent_C:theTime];
        }
    }
}
...
-(void)sendChatMsg:(NSString*)toId Content:(NSString*)content ChatType:(NSString*)chattype ChatMark:(NSString*)chatmark{
    JZChat_c *li = [JZChat_c Jz_initialize];
    [li initWithInfo:@"JZChat_c" Chatid:@"0" Userid:[QiaokerenApplication getAccountNumber] Toid:toId Chatcontent:content UploadTime:[UtilsAll getFormatTime] Chattype:chattype Resourceindex:@"0" ChatMark:chatmark];
    NSDictionary * nsdic = [li toDictionary];
    JZDataHandle * jzDataHandle = [JZDataHandle initdatahandle];
    jzDataHandle.JZDatadelegate = self;
    [jzDataHandle sendObject:nsdic time:@"2015" protocol:123 label:@"JZChat_c"];
}
                 ...
                 classlevel:[[QiaokerenApplication getUserSelectData]qlevel]
                  ...
                  introduce:[NSString stringWithFormat:@"由[%@]讲解的课程",[[QiaokerenApplication getUserSelectData]qusername]]
                      topic:[[QiaokerenApplication getUserSelectData]qusername] // 群名称作为主题
                       ...
                       suit:@"所有人"];
    NSDictionary * dictionary = [addLesson toDictionary];
    NSLog(@"addlesson_c%@",dictionary);
    JZDataHandle * jzDataHandle = [JZDataHandle initdatahandle];
    jzDataHandle.JZDatadelegate = self;
    [jzDataHandle sendObject:dictionary time:time protocol:1 label:@"addLesson"];
}

-(void) sendAddLessonContent_C:(NSString*)buildTime{
    addLessonContent_C *addLessonContent = [addLessonContent_C instance2];
    
    [addLessonContent initWithInfo:@"addLessonContent_C"
                                 p:@"0"
                            UserID:[[QiaokerenApplication getUserSelectData] quserid]
                        UploadTime:buildTime
                           classid:_thisClassId
                            userid:[[QiaokerenApplication getUserSelectData] quserid]
                         timestamp:buildTime
                          username:[[QiaokerenApplication getUserSelectData] qusername]
                             place:[UtilsAll getLocation]
                      contentclass:@"recording..."
                           content:[NSString stringWithFormat:@"此课程由%@精心录制",[[QiaokerenApplication getUserSelectData] quserid]]
                               url:[QiaokerenApplication getAudioJiaoXueUrlUserID:[[QiaokerenApplication getUserSelectData]quserid] TimeStamp:buildTime]
                               tip:@"0"];
    
    NSDictionary * dictionary = [addLessonContent toDictionary];
    JZDataHandle * jzDataHandle = [JZDataHandle initdatahandle];
    jzDataHandle.JZDatadelegate = self;
    [jzDataHandle sendObject:dictionary time:buildTime protocol:1 label:@"addLessonContent_C"];
}
...
- (void) dealLabel:(NetUnit *)nUnit{
    if ([nUnit->cLabel isEqualToString:@"JZChat_c"]) {
        NSLog(@"receive JZChat_s",nil);
    }
    if ([nUnit->cLabel isEqualToString:@"addLesson"]) {
        ...
        if ([[dictionary objectForKey:@"Mark"]isEqualToString:@"1"]) {
            ...
            [self showHint:@"您已进入讲课录制模式！"];
            [_recordButton setImage:[UIImage imageNamed:@"delete"] forState:(UIControlStateNormal)];
            NSString * classid = [dictionary objectForKey:@"classid"];
            NSLog(@"classid : %@",classid);
            _thisClassId = classid;
        } else {
            NSLog(@"发送失败 serverContent : %@",[dictionary objectForKey:@"Content"]);
        }
    }
    if ([nUnit->cLabel isEqualToString:@"addLessonContent_C"]) {
        NSString * json = nUnit->receiveString;
        NSDictionary * dictionary = [[NSJSONSerialization JSONObjectWithData:[json dataUsingEncoding:NSUTF8StringEncoding] options:NSJSONReadingMutableContainers error:nil] objectAtIndex:0];
        if ([[dictionary objectForKey:@"Mark"] isEqualToString:@"1"]) {
            NSString * lessonContentId = [dictionary objectForKey:@"classcontentid"];
            NSLog(@"lessonContentId : %@",lessonContentId);
        } else {
            NSLog(@"发送语音失败",nil);
        }
    }
