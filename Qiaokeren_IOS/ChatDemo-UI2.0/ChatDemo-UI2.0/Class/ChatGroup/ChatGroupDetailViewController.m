    ...
    for (i = 0; i < row; i++) {
        for (j = 0; j < kColOfRow; j++) {
            ...
            if (index < [self.dataSource count]) {
                ...
                NSString *title = [ContactsViewController getNickNameByUserId:username];
                if (title != nil && ![title isEqualToString:@""]) {
                    contactView.remark = title;
                }
                
                NSString *url = [QiaokerenApplication getTouXiangUrlUserID:username];
                [contactView.imageView setImageFromUrl:url
                                            completion:^(void) {}];
                
                //点击事件
                contactView.imageView.restorationIdentifier = username;
                contactView.imageView.userInteractionEnabled = YES;
                // 内容模式
                contactView.imageView.clipsToBounds = YES;
                contactView.imageView.contentMode = UIViewContentModeScaleAspectFill;
                [contactView.imageView addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(showOtherPersonInfoInGroupDetail:)] ];
...
-(void)showOtherPersonInfoInGroupDetail:(UITapGestureRecognizer *)imageTap{
